...
| `model_response` | step | raw provider response when a real adapter's output differs from the normalized action; for provider `external`, the raw response(s) the outside agent forwarded before that step's move (a list under `responses` when there was more than one) |
