...
| **Evaluation Systems** | `trace_harness/runner/`, `trace_harness/models/` (adapter contract, live adapters), `cli.py`; `trace_harness/environment/` (state, tools, retrieval, registry, guardrails, controls), `fixtures/docs/`; `trace_harness/tracing/` (events, recorder, artifact store, run index), `config.py`, the read path, the hosted public results and the local API plan (`docs/public_results.md`), data contracts to the frontend | Rupert Maiti, Evan Yang, Samrath |
