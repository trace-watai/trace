...
| Run config/result | `RunConfig 0.3.0`, `RunResult 0.1.0` | CLI, index, reader, batch, dashboard fixture | Each child run retains a normal run directory and effective configuration, including optional cassette settings and the live `call_policy` it ran under (null for fixture and replay runs). | Compatible; `0.1.0` and `0.2.0` configs remain readable, and every retained one is loaded in a test. |
...
| Suite config/summary | `Suite 0.3.0`, `BatchSummary 0.3.0` | CLI, delivery reporting | Child run IDs, verdict counts, termination counts, cost coverage, and canonical artifacts are emitted and tested. A suite may set `max_cost_usd`; the summary's `budget` block records spend, `budget_exhausted` or `budget_unenforceable`, and the cells never run. | Compatible; older suites and summaries remain readable and run uncapped. Unknown live-provider cost remains `null` and is never counted as zero by the budget guard. Fixture and replay runs cost exactly zero, and so does a live run that got no answer when every failed attempt carried an HTTP status. |
