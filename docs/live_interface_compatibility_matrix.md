...
| Suite config/summary | `Suite 0.2.0`, `BatchSummary 0.1.0` | CLI, delivery reporting | Child run IDs, verdict counts, termination counts, cost coverage, and canonical artifacts are emitted and tested. | Compatible; `0.1.0` suites remain readable. Unknown live-provider cost remains `null` rather than being reported as zero. |
